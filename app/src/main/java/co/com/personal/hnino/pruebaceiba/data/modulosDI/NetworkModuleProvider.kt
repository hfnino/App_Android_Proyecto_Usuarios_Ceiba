...
import co.com.personal.hnino.pruebaceiba.data.network.PostsUsersApiClientInterface
import co.com.personal.hnino.pruebaceiba.data.network.UsersApiClientInterface
...
// Este modulo, nos va a proveer dependencias (necesario para usar inyección de dependencias con Dagger Hiil)
// que no son faciles de proveer, lo anterior teniendo en cuenta que las dependencias faciles de proveer son
// las de clases que nosotros creamos en nuestro proyecto por ejemplo una clase llamada personas que tiene
// varios atributos y que podemos habilitarla para ser inyectada  en otras clases con @Inyect constructor(),
// mientras que las que no son faciles de proveer son las dependencias de librerias como retrofit o dependencias
// de clases que contienen interfaces, a las cuales no podemos habilitarlas para ser inyectadas con
// @Inyect constructor(), entonces nos es necesario crear modulos que se puedan proveer
...
object NetworkModuleProvider { // Los objetos pueden ser accedidos de manera directa, como si fueran variables o funciones dentro de un
    ...
    fun provideUsersRetrofit(): Retrofit { // retorna un objeto de tipo Retrofit. Como esta función esta configurada como @Provides, significa
        ...
        //a esta funcion, que retorna un objeto de Retrofit y la va a proveer.
        ...
        // que seria la clase UsersFromApi etc
    ...
    @Singleton // Usamos el patron de diseño Singleton para mantener una unica instancia de la interfaz
    // ItemTipoIdentifApiClientInterface, de tal manera que no se creen varias instancias cada vez que llamemos
    // a la interfaz ItemTipoIdentifApiClientInterface y con esto evitar consumir recursos de memoria sin tener
    // la necesitad.
    @Provides // Con esta etiqueta habilitamos este modulo para que => si cualquiera de nuestras clases necesita
    // una instancia de la interfaz ItemTipoIdentifApiClientInterface, Dagger Hilt va a saber como darsela, es
    // decir se va a poder inyectarsela en la clase que necesite esta dependencia.
    fun provideUsersApiClient(retrofit: Retrofit): UsersApiClientInterface {
        // Esta funcion retorna un objeto de tipo UsersApiClientInterface. Como esta función esta
        // configurada como @Provides, significa que si en cualquier clase necesitamos una instacia de
        // interfaz UsersApiClientInterface, simplemente la inyectamos, y Dagger Hilt va a buscar
        // y a encontrar a esta funcion, que retorna un objeto de UsersApiClientInterface y la va a proveer.

        // (retrofit: Retrofit) es proveido por la función provideUsersRetrofit existente en este mismo modulo.
        // Esto Funciona debido a que la función provideUsersRetrofit es una función @Provides y esa nos provee
        // retrofit para inyectarla en cualquier clase que la requiera y tambien se puede proveer en otras
        // funciones @Provides como sucede en este caso

        return retrofit.create(UsersApiClientInterface::class.java) // Se retorna un objeto de tipo
        // UsersApiClientInterface. el cual es un retrofit mesclado con la interfaz,es decir ya va el
        // retrofit completo.
    }

    @Singleton
    @Provides
    fun providePostsUsersApiClient(retrofit: Retrofit): PostsUsersApiClientInterface{
        return retrofit.create(PostsUsersApiClientInterface::class.java) // Se retorna un objeto de tipo
        // PostsUsersApiClientInterface. el cual es un retrofit mesclado con la interfaz,es decir ya va el
        // retrofit completo.
    }
